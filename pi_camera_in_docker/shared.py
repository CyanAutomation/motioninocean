...
def register_shared_routes(app: Flask, state: dict, get_stream_status: Optional[Callable[[], dict]] = None) -> None:
    @app.route('/health')
    def health():
        return jsonify({'status': 'healthy', 'timestamp': datetime.now().isoformat(), 'app_mode': state['app_mode']}), 200

    @app.route('/ready')
    def ready():
        if state['app_mode'] != 'webcam_node':
            return jsonify({'status': 'ready', 'reason': 'camera_disabled_for_management_mode', 'app_mode': state['app_mode'], 'timestamp': datetime.now().isoformat()}), 200
    ...
    @app.route('/metrics')
    def metrics():
        status = get_stream_status() if get_stream_status else {
            'frames_captured': 0,
            'current_fps': 0.0,
