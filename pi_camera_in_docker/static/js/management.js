...
function renderOverviewPanel() {
    if (overviewSnapshot) {
        ...
        setTextContent(overviewStreamingWebcams, String(overviewSnapshot.stream_available_webcams ?? 0));
    }
    if (overviewActivityList instanceof HTMLElement) {
        if (!activityFeed.length) {
            ...
            overviewActivityList.innerHTML = activityFeed
                .map((entry) => `<li><strong>${escapeHtml(new Date(entry.timestamp).toLocaleTimeString())}</strong> ${escapeHtml(entry.message)}</li>`)
                .join("");
        }
    }
    if (overviewActionList instanceof HTMLElement) {
        const statusEntries = Array.from(webcamStatusMap.values());
        const authIssues = statusEntries.filter((status) => String(status.error_code || "").toUpperCase() === "WEBCAM_UNAUTHORIZED").length;
        const privateIpBlocked = statusEntries.filter((status) => String(status.error_code || "").toUpperCase() === "SSRF_BLOCKED").length;
...
async function refreshStatuses({ fromInterval = false } = {}) {
    const statusHistoryMap = typeof previousStatusByNode !== "undefined" && previousStatusByNode instanceof Map
    ...
    try {
        do {
            ...
            const statusResult = await fetchStatusesForNodes(activeNodeIdsAtPollStart, allowManualFeedback, () => {
            ...
            if (currentToken === statusRefreshToken) {
                ...
                const filteredNextStatusMap = new Map(Array.from(nextStatusMap.entries()).filter(([nodeId]) => latestNodeIds.has(nodeId)));
...
function recordStatusHistory(nextStatuses, history) {
    for (const [nodeId, nextStatus] of nextStatuses.entries()) {
        ...
        else if (((prevCode !== nextCode && nextCode) ||
