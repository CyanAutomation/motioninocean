...
export function renderConfig(data, context) {
    const { state, setConfigValue, formatBoolean, formatUptime, applyMockStreamMode, setHealthIndicator, normalizeHealthState, healthText, updateConnectionDisplays, } = context;
...
function renderCameraSettings(cameraSettings, setConfigValue) {
    if (!cameraSettings)
        return;
    setConfigValue("config-resolution", cameraSettings.resolution
        ? `${cameraSettings.resolution[0]} × ${cameraSettings.resolution[1]}`
        : "--");
    setConfigValue("config-fps", cameraSettings.fps !== undefined ? `${cameraSettings.fps} FPS` : "--");
    setConfigValue("config-target-fps", cameraSettings.target_fps !== undefined ? `${cameraSettings.target_fps} FPS` : "--");
    setConfigValue("config-jpeg-quality", cameraSettings.jpeg_quality !== undefined ? `${cameraSettings.jpeg_quality}%` : "--");
}
function renderStreamControl(streamControl, state, setConfigValue, updateConnectionDisplays) {
    ...
    setConfigValue("config-max-frame-age", streamControl.max_frame_age_seconds !== undefined
        ? `${streamControl.max_frame_age_seconds}s`
        : "--");
    setConfigValue("config-cors", typeof streamControl.cors_origins === "string" && streamControl.cors_origins.length > 0
...
function renderRuntime(runtime, setConfigValue, formatBoolean, formatUptime, applyMockStreamMode) {
    ...
    applyMockStreamMode(runtime.mock_camera === true || runtime.active_mock_fallback === true, runtime.active_mock_fallback === true);
}
