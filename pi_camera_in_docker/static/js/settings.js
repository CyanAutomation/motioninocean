...
const SettingsUI = (() => {
    ...
    const updateSaveStatus = (metadata) => {
        ...
        saveStatus().classList.remove("hidden", "settings-save-status--saved", "settings-save-status--failed", "settings-save-status--restart-required");
    ...
    const buildPendingChanges = () => {
        ...
        for (const fieldKey of dirtyFields) {
            ...
            const input = document.querySelector(`.setting-input[data-category="${category}"][data-property="${property}"]`);
            if (!input) {
                continue;
            }
