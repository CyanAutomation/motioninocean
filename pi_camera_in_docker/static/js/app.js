...
async function fetchReadmeContent() {
    ...
    const normalizedPayload = {
        status: payload && typeof payload.status === "string" && payload.status.trim()
            ? payload.status
            : response.ok
                ? "ok"
                : "error",
        content: payload && typeof payload.content === "string" ? payload.content : "",
        message: payload && typeof payload.message === "string" && payload.message.trim()
            ? payload.message
            : response.ok
                ? ""
                : "Failed to load help documentation",
        documentation_url: payload && typeof payload.documentation_url === "string" ? payload.documentation_url : "",
...
function trapUtilityModalTabCycle(event) {
    ...
    const focusableElements = state.elements.utilityModal.querySelectorAll('a[href], area[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])');
...
function renderChangelogHtml(payload) {
    ...
    const cardsHtml = visibleEntries
        ...
        .join("");
    const statusNote = payload.status === "degraded" ? renderDegradedChangelogNote(payload, debugModeEnabled) : "";
    const fullChangelogUrl = typeof payload.full_changelog_url === "string" ? payload.full_changelog_url.trim() : "";
...
function renderDegradedChangelogNote(payload, debugModeEnabled) {
    const userFriendlyMessage = "We couldn't load the latest changelog details right now. Try again shortly.";
    if (!debugModeEnabled) {
...
function assertSinglePollingMode() {
    ...
    console.assert(!(statsPollingActive && configPollingActive), "Invalid polling state: stats (SSE) and config polling are both active.");
}
/**
...
async function updateStats() {
    if (state.statsInFlight)
        return;
    if (state.statsCollapsed || document.hidden)
        return;
    try {
...
function setUpdateFrequency(nextFrequency) {
    if (state.updateFrequency === nextFrequency)
        return;
...
function increaseBackoff() {
    ...
    const nextFrequency = Math.min(state.baseUpdateFrequency * Math.pow(2, state.consecutiveFailures), state.maxUpdateFrequency);
...
function applyMockStreamMode(isMockModeActive, isFallbackActive) {
    ...
    if (isMockModeActive) {
        setConnectionStatus("inactive", isFallbackActive ? "Mock fallback active (camera unavailable)" : "Mock camera mode active");
    }
}
const HEALTH_TEXT = {
    ok: "OK",
...
function normalizeHealthState(stateValue) {
    ...
    if (["ok", "pass", "healthy", "ready"].includes(normalized))
        return "ok";
    if (["warn", "warning", "degraded"].includes(normalized))
        return "warn";
    if (["fail", "error", "failed", "down", "unhealthy"].includes(normalized))
        return "fail";
    return "unknown";
}
...
function collectSetupConfig() {
    return {
        ...
        max_connections: parseInt(document.getElementById("setup-max-connections")?.value || "10", 10) || 10,
...
function setWizardStep(step) {
    if (!setupWizard.steps.includes(step))
        return;
...
function onSetupNext() {
    if (!validateStep(setupWizard.currentStep))
        return;
