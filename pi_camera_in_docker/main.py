...
def _collect_current_config() -> Dict[str, Any]:
    ...
    app_mode = os.environ.get("MIO_APP_MODE", DEFAULT_APP_MODE).strip().lower() or DEFAULT_APP_MODE
    ...
    auth_token_env_var = (
        ...
        if app_mode == "management"
        else "MIO_WEBCAM_CONTROL_PLANE_AUTH_TOKEN"
    )
...
def _generate_env_content(config: Dict[str, Any]) -> str:
    ...
    """
    selected_app_mode = str(config.get("app_mode", "webcam")).strip().lower() or "webcam"

    selected_auth_token = config.get("auth_token", "")
    ...
    env_lines = [
        ...
        f"MIO_APP_MODE={selected_app_mode}",
        "",
    ...
    if selected_app_mode == "webcam":
    ...
    elif selected_app_mode == "management":
        env_lines.extend(
            [
                "# Management mode token (required for /api/nodes/* and /api/discovery/*)",
                f"MIO_MANAGEMENT_AUTH_TOKEN={selected_auth_token}",
                "# Webcam mode token unused in management mode",
        ...
        env_lines.extend(
            [
                "# Unknown app mode; emitting both tokens for compatibility",
                f"MIO_MANAGEMENT_AUTH_TOKEN={selected_auth_token}",
                f"MIO_WEBCAM_CONTROL_PLANE_AUTH_TOKEN={selected_auth_token}",
                "",
            ]
...
def _create_base_app(config: Dict[str, Any]) -> Tuple[Flask, Limiter, dict]:
    ...
    def api_setup_generate():
        ...
        try:
            ...
            setup_config = dict(data)
            setup_config["app_mode"] = setup_config.get("app_mode", config["app_mode"])
            env_content = _generate_env_content(setup_config)
