  ...
  try {
    ...
    if (formExists) {
      ...
      if (await submitButton.isVisible()) {
        ...
        const feedback = await page.locator('#form-feedback').innerText().catch(() => '');
        if (feedback) {
    ...
    if (tableExists) {
      ...
      if (nodeCount === 0) {
        const emptyMsg = await page.locator('.empty-state-message').innerText().catch(() => '');
...
function generateReport(webcamFindings, managementFindings) {
  ...
  report += `## Summary\n\n`;
  const totalIssues = (webcamFindings.layout.length || 0) +
