...
async function runTest() {
  ...
  try {
    ...
    await takeAndLogScreenshot(page, 'desktop-02-config-tab-initial', 'Config tab initial state (desktop)');
    ...
    for (let i = 0; i < Math.min(2, toggleButtons.length); i++) {
      ...
      const contentBefore = await button.locator('..').locator('.config-group-content').boundingBox();
      
      // Click toggle to collapse
      await button.click();
      await page.waitForTimeout(200);
      
      const contentAfter = await button.locator('..').locator('.config-group-content').boundingBox();
    ...
    await takeAndLogScreenshot(page, 'desktop-03-config-collapsed', 'Config tab with collapsed sections (desktop)');
    ...
    await takeAndLogScreenshot(page, 'desktop-04-keyboard-focus', 'Keyboard focus state on toggle button');
    ...
    assessments['Information Clarity'].push('✓ Sections use emoji + text labels (good visual indicators)');
    
    // Check grouping
    assessments['Organization'].push('✓ Config items grouped into logical sections (Camera, Stream, Runtime, Limits)');
    ...
    assessments['Visual Hierarchy'].push(`✓ Headers have larger font (${headerFontSize}) than values`);

    // Check toggle feedback
    assessments['Interactivity Feedback'].push('✓ Toggle buttons show visual state changes');

    // Mobile experience
    assessments['Mobile Experience'].push(mobileOverflow ? '⚠ Horizontal scroll on mobile' : '✓ No horizontal scroll on mobile');
    assessments['Mobile Experience'].push(minFont >= 12 ? '✓ Font sizes readable on mobile' : '⚠ Font sizes may be too small');
